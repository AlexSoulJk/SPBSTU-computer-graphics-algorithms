...
HRESULT Render::InitBufferShader(ModelFactory::ModelCode code)
{
    ...
    if (SUCCEEDED(hr))
    {
        hr = CompileShader(L"PixelLight.ps", nullptr, &m_pLightPixelShader, &psBlobLight);
    }
    ...
    hr = DirectX::CreateDDSTextureFromFile(m_pDevice, L"cube_normal.dds", nullptr, &m_pNormalMapView);
    if (FAILED(hr))
        return hr;
...
void Render::RenderCubes(float aspecto)
{
    ...
    lights[1].Range = 3.0f;
    lights[1].Color = XMFLOAT3(0.5f, 0.0f, 1.0f);
    lights[1].Intensity = 3.0f;

    D3D11_MAPPED_SUBRESOURCE mappedResourceLight;
    HRESULT hr = m_pDeviceContext->Map(m_pLightBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResourceLight);
    if (SUCCEEDED(hr))
    {
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        m_pDeviceContext->UpdateSubresource((m_currentModel->m_pModelBuffer), 0, nullptr, &lightModelT, 0, 0);
